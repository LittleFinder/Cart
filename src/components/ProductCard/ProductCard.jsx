...
const ProductCard =({imgURL, title, price, quantity}) =>{
  const { products, addToCart, removeFromCart, favorited, removeFromFavorite, addToFavorite } = useShop();
  ...
  return (
    <div className={styles.card}>
      ...
      {isInFavorite ? <UnCheckFavorite className={styles.favoriteIcon}/>:<CheckFavorite className={styles.favoriteIcon}/>}
      ...
      <div className={styles.conteiner}>
        ...
        <button  onClick={handleClickAdd} className={`${styles.button} ${isInCart ? styles.checked : styles.add}`}>
            {isInCart ? <AddedIcon className="icon" height={11} width={11}/>:<PlusIcon className="icon" height={11} width={11}/>}
        </button>
      </div>
    </div>
  );
